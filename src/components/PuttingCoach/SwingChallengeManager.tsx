  ...
  return (
    <View style={styles.sideSummaryContainer}>
      ...
      <Text style={styles.sideSummaryRemaining}>
        {hasOvershot && '↩ '}
        {displayRemaining.value} {displayRemaining.unit} remaining
      </Text>
    </View>
  );
};
        ...
        {progress.shotHistory.map((shot, index) => (
          <View key={index} style={styles.shotHistoryRow}>
            <Text style={styles.shotHistoryText}>
              Shot {shot.stroke}: {CLUB_DATA[shot.club]?.shortName || 'PUTT'} -{' '}
...
const styles = StyleSheet.create({
  ...
  sideSummaryContainer: {
    ...
    padding: 12,
    minWidth: 100,
    zIndex: 100, // Lower z-index so settings menu appears above
  },
  sideSummaryTitle: {
    color: '#999',
    fontSize: 10,
